...
def draw_avl(screen, tree, font_m, font_s, info):
  ...
  def find_root(tree_dict: dict):
    ...
    for k, v in tree_dict.items():
      if v["left"]: children.add(v["left"])
      if v["right"]: children.add(v["right"])
    return list(all_nodes - children)[0]

  root = find_root(tree)

  # ------ layout position ------
  def layout_tree(tree_dict: dict, root_key: Node, screen_width, top_y=200, level_gap=120):
    pos = {}
  ...
  rect = text.get_rect(topleft=(60, UI_HEIGHT + 50))
  screen.blit(text, rect)
  if info.get("action") == "starting_insert":
    key = info["key"]   
    node = font_s.render(str(key), True, (255, 255, 255))
    screen.blit(node, node.get_rect(topleft=(rect.width + 60 + 10, UI_HEIGHT + 50)))    # the number being inserted
  else:
    none_text = font_s.render("None", True, (255, 255, 255))
    screen.blit(none_text, none_text.get_rect(topleft=(rect.width + 60 + 10, UI_HEIGHT + 50)))
